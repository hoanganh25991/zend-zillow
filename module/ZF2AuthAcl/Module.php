...
class Module{
    ...
    function boforeDispatch(MvcEvent $event){
        ...
        if($session->offsetExists('email')){
            if($requestedResourse == 'ZF2AuthAcl\Controller\Index-index' ||
                ...
                $status =
                    $acl->isAccessAllowed($userRole, $controller, $action);
                if(!$status){
        ...
        return array(
            'factories' => array(
                'AuthService' => function($serviceManager){
                    ...
                    $dbAuthAdapter =
                        new DbAuthAdapter($adapter, 'users', 'email',
                            'password');
